...
import org.bigbear.impressweibo.bean.android.MessageTimeLineData;
import org.bigbear.impressweibo.bean.android.TimeLinePosition;
import org.bigbear.impressweibo.dao.maintimeline.TimeLineReCmtCountDao;
import org.bigbear.impressweibo.othercomponent.WifiAutoDownloadPictureRunnable;
import org.bigbear.impressweibo.support.database.FriendsTimeLineDBTask;
import org.bigbear.impressweibo.support.debug.AppLogger;
import org.bigbear.impressweibo.support.error.WeiboException;
import org.bigbear.impressweibo.support.lib.HeaderListView;
import org.bigbear.impressweibo.support.lib.LogOnExceptionScheduledExecutor;
...
import org.bigbear.impressweibo.ui.adapter.TestAdapter;
...
import org.bigbear.impressweibo.ui.loader.FriendsMsgLoader;
import org.bigbear.impressweibo.ui.main.LeftMenuFragment;
import org.bigbear.impressweibo.ui.main.MainTimeLineActivity;
import org.bigbear.impressweibo.ui.send.WriteWeiboActivity;
...
//            ((MainTimeLineActivity) getActivity()).setCurrentFragment(this);
    ...
    protected void buildListAdapter() {
    	AppLogger.d("LIST "+getList().getItemList().toString());
        WeiboAdapter adapter = new WeiboAdapter(this,getList().getItemList());
        //TestAdapter adapter=new TestAdapter(this,getList().getItemList());
    ...
    public void buildActionBarNav() {
        ...
        if (Utility.isDevicePort()) {
            ...
            ((MainTimeLineActivity) getActivity()).setTitle("");
            getActivity().getActionBar().setIcon(R.drawable.icon_final);
        }
    ...
    protected void newMsgLoaderSuccessCallback(MessageListBean newValue, Bundle loaderArgs) {
    	AppLogger.d("newMsgLoaderSuccessCallback");
        if (Utility.isAllNotNull(getActivity(), newValue) && newValue.getSize() > 0) {
            if (loaderArgs != null && loaderArgs
            	...
            	AppLogger.d("addNewDataAndRememberPositionAutoRefresh");
                addNewDataAndRememberPositionAutoRefresh(newValue);
            } else {
            	AppLogger.d("addNewDataWithoutRememberPosition");
        ...
        if (getActivity() != null && newValue.getSize() > 0) {
            ...
            /*if (isFirstViewHeader && !headerListView.isInTouchByUser()) {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    int firstAdapterItemPosition = 0;
                    int currentAdapterItemPosition = firstAdapterItemPosition + i;
    ...
    private void addNewDataWithoutRememberPosition(MessageListBean newValue) {
        ...
        getList().addNewData(newValue);
        AppLogger.d("getList(): "+getList().getTotal_number()+"|");
        getAdapter().notifyDataSetChanged();
        progressBar.setVisibility(View.GONE);
        recyclerView.setVisibility(View.VISIBLE);
        if (recyclerView.getAdapter() instanceof TestAdapter) AppLogger.d("true TestAdapter"+recyclerView.getAdapter().getItemCount());
